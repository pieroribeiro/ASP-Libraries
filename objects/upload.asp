...
Class FreeASPUpload
	...
	Private DEFAULT_ASP_CHUNK_SIZE
	...
	Private BYTES_PER_IMAGE

	Private Sub Class_Initialize()
		BYTES_PER_IMAGE = 512000
		DEFAULT_ASP_CHUNK_SIZE = 200000
		adModeReadWrite = 3
		adTypeBinary = 1
		adTypeText = 2
	...
	Public Sub Save(path)
		Dim streamFile, fileItem, filePath, tmpPath

		if Right(path, 1) <> "\" then path = path & "\"

		if not uploadedYet then Upload

		tmpPath = Server.MapPath("/static/inscricoes/imagens_inscricoes/TMP_IMAGES/")
		If Right(tmpPath, 1) <> "\" Then tmpPath = tmpPath & "\"
		
		For Each fileItem In UploadedFiles.Items
			'filePath = path & fileItem.FileName
			If UCase(""& ExtensionFile(fileItem.FileName)) = ".JPG" Then
				If fileItem.Length <= BYTES_PER_IMAGE Then
					tmp_pathArray = Split(path,"\")
					If IsArray(tmp_pathArray) Then 
						tmp_filename	= GeraHash() & tmp_pathArray(UBound(tmp_pathArray)-1) & ExtensionFile(fileItem.FileName)
					Else
						tmp_filename	= GeraHash() & ExtensionFile(fileItem.FileName)
					End If
					TMPImage		= tmpPath & tmp_filename
					fileItem.FileName = tmp_filename
					filePath = tmpPath & tmp_filename
				...
				Else
					Response.Cookies("StringRetorno")		= "Upload permitido apenas de imagens de "& (BYTES_PER_IMAGE / 1024) &" Kb"
					Response.Redirect("default.asp?f=s")
					Exit For
				End If
			Else
				Response.Cookies("StringRetorno")		= "Upload permitido apenas para imagens JPG"
				Response.Redirect("default.asp?f=s")
				Exit For
			End If
		 Next
	End Sub	
	Private Function ExtensionFile(fileName)
		If InStr(fileName, ".") Then
			strExtensao				= LCase(Mid(fileName, InStrRev(fileName, ".")))
		End If
		
		ExtensionFile = strExtensao
	End Function	
	Private Function GeraHash()
		Randomize()
		Dim dia, mes, ano, hora, minuto, segundo, random
		...
		random		= Right("00000"& Int(RND() * 10000), 4)
		
		GeraHash	= ano & mes & dia & hora & minuto & segundo & random
	End Function	
